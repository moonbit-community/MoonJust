...
| PR-064 | Native CLI composition, Wasm read-only adapter and deny-by-default inspect policy | `src/host_wasm`, `policies/inspect.toml`, `tools/check_inspect_policy.sh` |
...
- `tools/differential/real_smoke.sh`: 4 matches, 6 classified differences,
  0 failures.
- `tools/check_inspect_policy.sh`: pass.
- `tools/check_query_compat.sh`: 24 Native/Wasm query cases match the pinned
