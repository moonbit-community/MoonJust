...
- Strict review: 2026-08-07 ([Phase 0-9 audit](RELEASE_AUDIT.md))
...
- `tests/upstream/just-1.57.0/builtins.jsonl` has one machine-checked row
  per builtin (`min_arguments`, `max_arguments`, aliases, purity, capabilities,
  Native/wasm1 targets, evidence and tracking ID).
- `tests/upstream/just-1.57.0/evaluator-oracle.jsonl` contains 20 pinned Rust
  SemVer/regexp outcomes; `tools/upstream/evaluator_oracle.py` executes them against
...
- `tools/upstream/evaluator_oracle.py`: 20/20 pinned Rust oracle cases passed.
...
recorded in [`RELEASE_AUDIT.md`](RELEASE_AUDIT.md). This does not change the
...
commit evidence is recorded in [`RELEASE_AUDIT.md`](RELEASE_AUDIT.md).
