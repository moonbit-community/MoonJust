...
The stable facade is the root `moonbit-community/MoonJust` package. Construct
...
test "format source through the stable facade" {
  ...
  let formatted = @MoonJust.format_source(source)
...
test "compile source through the stable facade" {
  ...
  let compilation = @MoonJust.compile_source(source)
...
The formal public surface is committed in `pkg.generated.mbti`. Run
