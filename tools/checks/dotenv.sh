...
CARGO_TARGET_DIR="$oracle_target" cargo build \
  --quiet \
  --locked \
  --manifest-path "$repo_root/tools/oracles/dotenvy/Cargo.toml"
...
compare() {
  ...
  moon run --quiet --target native tools/probes/dotenv_probe \
...
expect_invalid() {
  ...
  moon run --quiet --target native tools/probes/dotenv_probe <"$fixture" \
