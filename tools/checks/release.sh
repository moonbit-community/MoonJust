...
work=$(mktemp -d "${TMPDIR:-/tmp}/moonjust-release.XXXXXX")
...
cleanup() {
  case "$work" in
    "${TMPDIR:-/tmp}"/moonjust-release.*) rm -rf -- "$work" ;;
  esac
}
...
echo "Release engineering gate passed: metadata, policies, package, artifacts, MoonX and supply chain"
