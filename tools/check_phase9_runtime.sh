...
fixture="$repo_root/tests/fixtures/phase-9"
work=$(mktemp -d "${TMPDIR:-/tmp}/moonjust-phase9-runtime.XXXXXX")
trap 'rm -rf -- "$work"' EXIT HUP INT TERM

fail() {
  echo "Phase 9 runtime gate failed: $1" >&2
  exit 1
}
...
echo "Phase 9 runtime gate passed (bounded scheduler, crash recovery, two-process contention, Native/wasm cache CLI)"
