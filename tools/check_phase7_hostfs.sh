...
binary="$repo_root/_build/wasm/debug/build/tools/phase7_hostfs_probe/phase7_hostfs_probe.wasm"
allow_policy="$repo_root/policies/filesystem.toml"
deny_policy="$repo_root/policies/inspect.toml"
work=$(mktemp -d "${TMPDIR:-/tmp}/moonjust-phase7-hostfs.XXXXXX")
...
moon build --target wasm tools/phase7_hostfs_probe
...
echo "Phase 7 HostFs policies verified: allowed atomic commit and typed denial"
