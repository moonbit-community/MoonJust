...
repo_relative="_build/phase7-workdir-cli.$$"
...
fail() {
  echo "Phase 7 working-directory differential failed: $1" >&2
  exit 1
}

"$repo_root/tools/upstream/build_oracle.sh" >/dev/null
moon build --quiet --target native tools/phase7_workdir_probe
probe="$repo_root/_build/native/debug/build/tools/phase7_workdir_probe/phase7_workdir_probe.exe"
...
echo "Phase 7 working-directory differential passed (9 model and 2 Native/Wasm CLI cases against just 1.57.0)"
