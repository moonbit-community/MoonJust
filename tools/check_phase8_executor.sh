...
fixture="$repo_root/tests/fixtures/phase-8/line.justfile"
expected="$repo_root/tests/fixtures/phase-8/line.dry-run.stderr"
work=$(mktemp -d "${TMPDIR:-/tmp}/moonjust-phase8-executor.XXXXXX")
trap 'rm -rf -- "$work"' EXIT HUP INT TERM

fail() {
  echo "Phase 8 executor gate failed: $1" >&2
  exit 1
}
...
echo "Phase 8 executor gate passed (oracle, native/wasm CLI corpus, and 15+15 executor cases)"
