...
compare() {
  ...
  moon run --quiet --target native tools/phase7_dotenv_probe \
  ...
  cmp "$work/$name.oracle" "$work/$name.candidate" || {
    ...
    echo "Phase 7 dotenv differential failed: $name" >&2
    exit 1
  }
}
...
expect_invalid() {
  ...
  moon run --quiet --target native tools/phase7_dotenv_probe <"$fixture" \
    >"$work/$name.stdout" 2>"$work/$name.stderr" || candidate_status=$?
  [ "$oracle_status" -ne 0 ] && [ "$candidate_status" -ne 0 ] || {
    echo "Phase 7 dotenv differential failed: invalid fixture status differs: $name" >&2
    exit 1
  }
}
...
if grep -q 'top-secret' "$work/invalid.stderr"; then
  echo "Phase 7 dotenv differential failed: diagnostic disclosed a value" >&2
  exit 1
fi
printf '\357\273\277BOM=rejected\n' >"$work/bom.env"
expect_invalid bom "$work/bom.env"

echo "Phase 7 dotenvy differential passed (6 fixtures, diagnostics redacted)"
