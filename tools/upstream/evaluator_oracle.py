#!/usr/bin/env python3
"""Verify committed evaluator builtin outcomes against pinned just."""
...
CORPUS = ROOT / "tests/upstream/just-1.57.0/evaluator-oracle.jsonl"
...
def main() -> int:
    ...
    if len(rows) != 20:
        raise SystemExit(f"evaluator oracle expected 20 rows, found {len(rows)}")
    seen: set[str] = set()
    for row in rows:
        case_id = row["id"]
        if case_id in seen:
            raise SystemExit(f"duplicate evaluator oracle id {case_id}")
        seen.add(case_id)
    ...
    print(f"evaluator Rust oracle verified: {len(rows)} cases")
    return 0
