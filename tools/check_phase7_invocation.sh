...
fixture="$repo_root/tests/fixtures/phase-7/invocation.justfile"
work=$(mktemp -d "${TMPDIR:-/tmp}/moonjust-invocation.XXXXXX")
trap 'rm -rf -- "$work"' EXIT HUP INT TERM

fail() {
  echo "Phase 7 invocation differential failed: $1" >&2
  exit 1
}

"$repo_root/tools/upstream/build_oracle.sh" >/dev/null
moon build --quiet --target native tools/phase7_invocation_probe
probe="$repo_root/_build/native/debug/build/tools/phase7_invocation_probe/phase7_invocation_probe.exe"
...
echo "Phase 7 invocation differential passed (11 argv and 3 Native/Wasm usage cases against just 1.57.0)"
